...
async function createPairAndAddLiquidity(
  ...
  const TokenAContractFactory =
    tokenAAddress === WNEONAddress
      ? await ethers.getContractFactory("WNEON")
      : await ethers.getContractFactory(contract);
  const TokenBContractFactory =
    tokenBAddress === WNEONAddress
      ? await ethers.getContractFactory("WNEON")
      : await ethers.getContractFactory(contract);
  ...
  if (balanceA < tokenAAmount) {
    ...
    console.log(
      `Minted ${ethers.formatUnits(
        ...
        tokenADecimals
      )} ${tokenASymbol} to deployer address ${deployer.address}`
    );
  ...
  if (allowanceA < tokenAAmount) {
    ...
    console.log(
      `Approved PancakeRouter to spend ${ethers.formatUnits(
        allowanceA.toString(),
        tokenADecimals
      )} ${tokenASymbol} on behalf of deployer`
    );
  }
  console.log(
    `PancakeRouter ${tokenASymbol} allowance: ${ethers.formatUnits(
      allowanceA.toString(),
      tokenADecimals
    )}`
  );
  ...
  if (balanceB < tokenBAmount) {
    ...
    console.log(
      `Minted ${ethers.formatUnits(
        amountToMint.toString(),
        tokenBDecimals
      )} ${tokenBSymbol} to deployer address ${deployer.address}`
    );
  ...
  if (allowanceB < tokenBAmount) {
    ...
    console.log(
      `Approved PancakeRouter to spend ${ethers.formatUnits(
        allowanceB.toString(),
        tokenBDecimals
      )} ${tokenBSymbol} on behalf of deployer`
    );
  }
  console.log(
    `PancakeRouter ${tokenBSymbol} allowance: ${ethers.formatUnits(
      allowanceB.toString(),
      tokenBDecimals
    )}`
  );
