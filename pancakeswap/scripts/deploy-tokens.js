...
async function main() {
  ...
  await deployERC20ForSPLMintable("token_X", "Token X", "TOKEN_X", 9, mintAuthority);

  await deployERC20ForSPLMintable("token_Y", "Token Y", "TOKEN_Y", 12, mintAuthority);
...
async function deployERC20ForSPLMintable(
  ...
  if (!config[tokenKey][network.name]) {
    ...
    token = await ERC20ForSPLMintableContractFactory.deploy(name, symbol, decimals, mintAuthority);
